...
case 18
    ...
    p.solver = 'Howard';
    p.prefix = sprintf('experiment_Energy_Howard_Expl_TS');
    p.theta = 0.0;

case 19

    p.solver = 'SL';
    p.prefix = sprintf('experiment_Energy_SL_TS');
    p.theta = 1.0;

case 20

    p.solver = 'Howard';
    p.prefix = sprintf('experiment_Energy_Howard_ref_Expl_TS');
...
case 21
    ...
    p.solver = 'SL';
    p.prefix = sprintf('experiment_Energy_SL_ref_TS');
    p.refinement.level = 1;
    p.refinement.mode = 'abs';
    p.refinement.xlow = [30 20];
    p.refinement.xup = [50 60];
...
% keyboard
fheader={'Nx', 'Ndofs', 't1460', 't730', 't365', 't183', 't91', 't46'};

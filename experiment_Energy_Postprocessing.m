...
p.outputdir = 'output_EnergyPostprocessing';
p.plotLevel  = 0;
...
case 18
    ...
    p.solver = 'Howard';
    p.prefix = sprintf('experiment_Energy_Howard_Expl_PP_long');
    p.theta = 0.0;

case 19

    p.solver = 'SL';
    p.prefix = sprintf('experiment_Energy_SL_PP_long');
    p.theta = 1.0;

case 20

    p.solver = 'Howard';
    p.prefix = sprintf('experiment_Energy_Howard_ref_Expl_PP_long');
...
case 21
    ...
    p.solver = 'SL';
    p.prefix = sprintf('experiment_Energy_SL_ref_PP_long');
    p.refinement.level = 1;
    p.refinement.mode = 'abs';
    p.refinement.xlow = [30 20];
    p.refinement.xup = [50 60];
...
% keyboard
fheader={'Level','Nx', 'Ndofs', 'Nt', 'val_HJB', 'val_orig', 'val_PP', 'improvement'};

% Out = zeros(length(Nxset),length(Lset)+1);
k = 1;
Out(:,1) = 1:length(Nxset);
...
for nx0 = Nxset
    ...
    for l = Lset
        ...
        % Determine simulated values for original control
        ...
        % Determine simulated values for postprocessed control
        Out(k,7) = valpost;

        % Relative improvement
        Out(k,8) = median(d_rel);

        % Write csv with convergence results
        fname = strcat(p.outputdir,'/', p.prefix, '_nx',string(p.nx), '.csv');
        fid = fopen(fname, 'w');
        fprintf(fid, '%s\n', strjoin(fheader,','));
        fprintf('Write Out table to %s\n',fname)

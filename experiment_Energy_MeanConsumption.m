...
p.dim = 3;
MeanC = 8000+1000*[-2,-1.5,-1,-0.5,0,0.5,1,1.5,2];
Lset = 0;
nx0 = 64;
p.nt = 365*4;
...
case 18
    ...
    p.solver = 'Howard';
    p.prefix = sprintf('experiment_Energy_Howard_Expl_Consumption_Study');
    p.theta = 0.0;

case 19

    p.solver = 'SL';
    p.prefix = sprintf('experiment_Energy_SL_Consumption_Study');
    p.theta = 1.0;

case 20

    p.solver = 'Howard';
    p.prefix = sprintf('experiment_Energy_Howard_ref_Expl_Consumption_Study');
...
case 21
    ...
    p.solver = 'SL';
    p.prefix = sprintf('experiment_Energy_SL_ref_Consumption_Study');
    p.refinement.level = 1;
    p.refinement.mode = 'abs';
    p.refinement.xlow = [30 20];
    p.refinement.xup = [50 60];
...
% keyboard
fheader={'Consumption', 'Nx', 'Ndofs', 'Nt', 'val'};

% Out = zeros(length(Nxset),length(Lset)+1);
k = 1;
Out(:,1) = MeanC;
Nxset = nx0*ones(size(MeanC));
for nx0 = Nxset
